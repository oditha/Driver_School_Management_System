...
public class ViewExamDayAttendent extends javax.swing.JFrame {

    static boolean maximized = true;

    public ViewExamDayAttendent() {
        initComponents();
        maximizeWindow();
        loadCombo();
    }
    ...
    private void initComponents() {
        ...
        setTitle("View Exam Day Attendence-New Kamal Learners | Rikillagaskada");
        ...
        jLabel2.setText("Select Exam Date");
        jPanel3.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 70, 120, 20));
        ...
        jPanel1.add(jPanel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 70, 550, 110));
        ...
        jPanel1.add(jScrollPane2, new org.netbeans.lib.awtextra.AbsoluteConstraints(330, 190, 800, 450));
    ...
    private void btnFilter2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnFilter2ActionPerformed
        try {
            ...
            if (checkPresent.isSelected()) {
                rs = DB.search("SELECT studentexam.*,examdayattendece.*,students.* FROM examdayattendece INNER JOIN students ON examdayattendece.idstudents = students.idstudents INNER JOIN studentexam ON studentexam.idstudents = students.idstudents WHERE ExamDate='" + getDate + "' AND Type LIKE 'Present' ");
            } else if (checkAbsent.isSelected()) {
                rs = DB.search("SELECT studentexam.*,examdayattendece.*,students.* FROM examdayattendece INNER JOIN students ON examdayattendece.idstudents = students.idstudents INNER JOIN studentexam ON studentexam.idstudents = students.idstudents WHERE ExamDate='" + getDate + "' AND Type LIKE 'Absent' ");
            } else {
                rs = DB.search("SELECT studentexam.*,examdayattendece.*,students.* FROM examdayattendece INNER JOIN students ON examdayattendece.idstudents = students.idstudents INNER JOIN studentexam ON studentexam.idstudents = students.idstudents WHERE ExamDate='" + getDate + "' ");
            }
            while (rs.next()) {
                Vector v = new Vector();
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ViewExamDayAttendent().setVisible(true);
            }
        });
    }
...
private void maximizeWindow() {
        if (maximized) {
            ...
            ViewExamDayAttendent.this.setExtendedState(JFrame.MAXIMIZED_BOTH);
            GraphicsEnvironment gv = GraphicsEnvironment.getLocalGraphicsEnvironment();
            ViewExamDayAttendent.this.setMaximizedBounds(gv.getMaximumWindowBounds());
            maximized = true;
        } else {
